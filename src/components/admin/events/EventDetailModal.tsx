...
export function EventDetailModal({ event, isOpen, onClose, onUpdate }: EventDetailModalProps) {
  ...
  async function fetchCreatorName(userId?: string) {
    if (!userId) { setCreatorName('Sistema'); return; }
  ...
  const displayStatus = event.status === 'cancelled' ? 'cancelled' : isPastEvent && event.status === 'confirmed' ? 'completed' : event.status;
  const isCancelled = displayStatus === 'cancelled'; // ✅ Detectar si está cancelado

  const formatDate = (dateString: string) => {
    if (!dateString) return '';
    return new Date(dateString).toLocaleDateString('es-PE', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true });
  };
    ...
    try {
      ...
      if (totalPaid > 0) {
        const { error: refundError } = await supabase.from('expenses' as any).insert({
            ...
            amount: totalPaid,
            category: 'devolucion', // ✅ CATEGORÍA CLAVE para Finanzas
            date: new Date().toISOString().split('T')[0],
            event_id: event.id
        });
        ...
        {showCancelModal && (
          <div className="absolute inset-0 z-50 bg-white/95 backdrop-blur flex flex-col items-center justify-center text-center p-8 animate-in fade-in">
            ...
            <p className="text-secondary-500 max-w-md mb-6">
                Estás a punto de cancelar el evento de <strong>{(event as any).client?.name}</strong>.
            ...
            <div className="space-y-6">
              ...
              <div className="bg-secondary-50 rounded-xl p-4 border border-secondary-100 space-y-3">
                <div className="flex items-center gap-2 text-xs text-secondary-500">
                  <Clock className="w-3 h-3" /><span>Creado: <span className="font-medium text-secondary-700">{formatDate(event.created_at)}</span></span>
                </div>
                {event.updated_at && (
                  <div className="flex items-center gap-2 text-xs text-secondary-500">
                    <Save className="w-3 h-3" /><span>Actualizado: <span className="font-medium text-secondary-700">{formatDate(event.updated_at)}</span></span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-xs text-secondary-500">
                  <UserCog className="w-3 h-3" /><span>Creado por: <span className="font-medium text-secondary-700">{creatorName || '...'}</span></span>
                </div>
              </div>
                ...
                <div className="bg-white p-5 rounded-xl border border-secondary-200 shadow-sm">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      <div className="p-3 bg-secondary-50 rounded-lg border border-secondary-100 relative group">
                         ...
                         {/* ✅ LÓGICA DE EDICIÓN: Solo Super Admin puede editar */}
                   ...
                   {/* ✅ BOTÓN OCULTO SI ESTÁ CANCELADO */}
