  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500 pt-10 pb-20">
      ...
      <div className="text-center space-y-2">
        <div className="w-16 h-16 bg-primary-50 text-primary-500 rounded-full flex items-center justify-center mx-auto mb-4"><FileSignature size={32}/></div>
        ...
        <div className="space-y-4">
            ...
            <Combobox value={selectedProposal} onChange={setSelectedProposal}>
              <div className="relative mt-1">
                <div className="relative w-full cursor-default overflow-hidden rounded-xl bg-gray-50 text-left border border-gray-200 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                ...
                <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm z-50">
                  {pendingProposals.length === 0 && query !== '' ? (
                    ...
                    pendingProposals.map((proposal) => (
                      <Combobox.Option key={proposal.id} className={({ active }) => `relative cursor-default select-none py-3 pl-10 pr-4 ${active ? 'bg-primary-600 text-white' : 'text-gray-900'}`} value={proposal}>
                        {({ selected, active }) => (
                          <>
                            <div className="flex flex-col">
                                <span className={`block truncate ${selected ? 'font-medium' : 'font-normal'}`}>{proposal.client_name}</span>
                                {/* Cambio de blue-200 a primary-200 */}
                                <span className={`block text-xs ${active ? 'text-primary-200' : 'text-gray-500'}`}>{proposal.event_type} • S/ {proposal.total_price}</span>
                            </div>
                            {selected ? <span className={`absolute inset-y-0 left-0 flex items-center pl-3 ${active ? 'text-white' : 'text-primary-600'}`}><Check className="h-5 w-5" aria-hidden="true" /></span> : null}
                          </>
                        )}
                      </Combobox.Option>
                    ))
            ...
            <div className="p-6 bg-gray-50 rounded-xl border border-gray-200 space-y-6">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div>
                   ...
                   <input type="number" value={contractConfig.extraHourCost} onChange={e => setContractConfig({...contractConfig, extraHourCost: Number(e.target.value)})} className="w-full p-2 border rounded-lg resize-none focus:ring-primary-500 focus:border-primary-500"/>
                 </div>
                 <div>
                   <label className="block text-xs font-semibold text-gray-500 mb-1">Depósito de Garantía (S/)</label>
                   <input type="number" value={contractConfig.guaranteeDeposit} onChange={e => setContractConfig({...contractConfig, guaranteeDeposit: Number(e.target.value)})} className="w-full p-2 border rounded-lg resize-none focus:ring-primary-500 focus:border-primary-500"/>
                 </div>
                 <div>
                   <label className="block text-xs font-semibold text-gray-500 mb-1">Horas de Montaje Previas</label>
                   <input type="number" value={contractConfig.setupHours} onChange={e => setContractConfig({...contractConfig, setupHours: Number(e.target.value)})} className="w-full p-2 border rounded-lg resize-none focus:ring-primary-500 focus:border-primary-500"/>
                 </div>
                 ...
                 <div>
                   ...
                   <input 
                     ...
                     className="w-full p-2 border rounded-lg resize-none focus:ring-primary-500 focus:border-primary-500"
                   />
                 </div>
               </div>
            </div>

            {/* CONFIGURACIÓN DE SUMINISTROS (Colores actualizados a primary) */}
            <div className="p-6 bg-primary-50 rounded-xl border border-primary-100 space-y-4">
               <h3 className="font-bold text-primary-900">3. Suministro de Licores e Insumos</h3>
               
               <RadioGroup value={supplyType} onChange={setSupplyType} className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <RadioGroup.Option value="provider" className={({ checked }) => `cursor-pointer rounded-lg px-4 py-3 border shadow-sm flex items-center justify-center text-sm font-medium transition-all ${checked ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-700 hover:bg-gray-50'}`}>
                    La Reserva pone todo
                  </RadioGroup.Option>
                  <RadioGroup.Option value="client_all" className={({ checked }) => `cursor-pointer rounded-lg px-4 py-3 border shadow-sm flex items-center justify-center text-sm font-medium transition-all ${checked ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-700 hover:bg-gray-50'}`}>
                    Cliente pone todo
                  </RadioGroup.Option>
                  <RadioGroup.Option value="client_partial" className={({ checked }) => `cursor-pointer rounded-lg px-4 py-3 border shadow-sm flex items-center justify-center text-sm font-medium transition-all ${checked ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-700 hover:bg-gray-50'}`}>
               ...
               {supplyType === 'client_partial' && (
                 <div className="mt-4 p-4 bg-white rounded-lg border border-primary-100 animate-in slide-in-from-top-2">
                   <label className="block text-xs font-bold text-gray-500 mb-2">Lista de insumos del cliente:</label>
                   <div className="flex gap-2 mb-2">
                       <input type="number" min="1" className="w-20 p-2 border rounded-lg resize-none focus:ring-primary-500 focus:border-primary-500 text-sm" placeholder="Cant." value={newItem.qty} onChange={e => setNewItem({...newItem, qty: Number(e.target.value)})}/>
                       <input type="text" className="flex-1 p-2 border rounded-lg resize-none focus:ring-primary-500 focus:border-primary-500 text-sm" placeholder="Descripción (ej. Botellas de Whisky)" value={newItem.description} onChange={e => setNewItem({...newItem, description: e.target.value})}/>
                       {/* Botón agregar actualizado */}
                       <button onClick={addClientItem} className="p-2 bg-primary-600 text-white rounded-lg"><Plus size={16}/></button>
                   </div>
