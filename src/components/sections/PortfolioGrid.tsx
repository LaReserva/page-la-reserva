...
// Extendemos el tipo para incluir 'category' en la vista pública
interface PublicImage extends EventImage {
...
export function PortfolioGrid({ images }: PortfolioGridProps) {
  ...
  const categories = useMemo(() => {
    // Usamos 'category' (el string). Si es null, lo mandamos a 'Otros'.
    const cats = new Set(images.map(img => img.category || 'Otros'));
    
    const uniqueCats = Array.from(cats).sort((a, b) => {
      if (a === 'Otros') return 1;
      if (b === 'Otros') return -1;
  ...
  const filteredImages = useMemo(() => {
    ...
    return images.filter(img => {
      const imgCat = img.category || 'Otros';
      return imgCat === activeFilter;
    });
  }, [images, activeFilter]);
  ...
  return (
    <div className="space-y-12">
        ...
        <AnimatePresence mode="popLayout">
          {filteredImages.map((img) => (
            <motion.div
              ...
              className="group relative aspect-[3/4] overflow-hidden rounded-xl bg-secondary-100 cursor-pointer shadow-sm hover:shadow-xl transition-shadow"
            >
              <img
                src={img.image_url}
      ...
      {filteredImages.length === 0 && (
        <div className="text-center py-20 bg-secondary-50 rounded-xl border border-dashed border-secondary-200">
          <p className="text-secondary-400 font-medium">No hay imágenes en esta categoría.</p>
        </div>
      )}
    </div>
  );
